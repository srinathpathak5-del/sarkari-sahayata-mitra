...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const filtered = schemesData.schemes.filter(s =>
    ...
    <nav className="bg-blue-800 text-white shadow-lg sticky top-0 z-50">
      <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
        ...
        <div ref={ref} className="relative hidden md:block flex-1 max-w-xs">
          ...
          {showSearch && results.length > 0 && (
            <div className="absolute top-full left-0 right-0 bg-white rounded-xl shadow-2xl mt-1 overflow-hidden z-50">
              {results.map(s => (
      ...
      {showSearch && (
        <div className="lg:hidden px-4 pb-3" ref={ref}>
          ...
          {results.length > 0 && (
            <div className="bg-white rounded-xl shadow-2xl mt-1 overflow-hidden">
              {results.map(s => (
                <button key={s.id} onClick={() => handleSelect(s.slug)}
                  className="w-full flex items-center gap-3 px-4 py-3 hover:bg-blue-50 transition border-b last:border-0 text-left">
                  <span className="text-xl">{s.icon}</span>
